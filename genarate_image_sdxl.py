...
def main():
    ...
    parser.add_argument("--workflow", default="sdxl_workflow_api.json", help="Workflow to use")
    parser.add_argument("--mode", type=int, default=2, help="Mode of operation")
    parser.add_argument("--output_path", default="C:/Users/lab929/Documents/ComfyUI-master/ComfyUI-master/output/T2I/sdxl/", help="Output directory for generated images")
        ...
        for file_name in description_list:
            with open(file_path + "/" + file_name, 'r', encoding='utf-8') as f:
                ...
                try:
                    ...
                    print(f"Error loading {file_name}")
            count +=1
    output_folder = args.output_path + "/gpt_4o_new/" + mode_string[1:]


    if mode == 3:
        output_folder = output_folder + "/" + step_string
    ...
    for count, description in enumerate(description_list):
        ...
        prompt["11"]["inputs"]["seed"] = random.randint(0, 4294967295)

        #set image filename
        prompt["19"]["inputs"]["filename_prefix"] = prefix + description + mode_string

        queue_prompt(prompt)
        
        #num = 1000
        #if count < num and count >= num-100:
        print("count: ", count)
